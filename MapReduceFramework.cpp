...
void set_counter(std::atomic<uint32_t>* atomic_counter,stage_t stage);
                            ...
                            int multiThreadLevel)
{
    ...
    for (int i = 0; i < multiThreadLevel; ++i)
    {
        if (pthread_create(&jobContext->threads[i], nullptr, wrappingFunc, &jobContext->thread_contexts[i]) != 0)
        {
...
void getJobState(JobHandle job, JobState* state)
{
    ...
    uint32_t cur_atomic = *(job_context->atomic_counter);
    uint32_t cur_count = (cur_atomic&0xFFFFFFFC);
    uint32_t cur_stage = ((static_cast<uint32_t>(0x30) << 30) & cur_atomic);
    state->percentage = cur_count/job_context->
...
void* mapping(void* arg)
{
    ...
    while (true)
    {
        uint32_t old_val = *(tc->job_context->atomic_counter);
        if (old_val >= inputVec.size())
        {
            break;
        }
        tc->job_context->client->map(inputVec[index].first, inputVec[index].second, tc);
    }
                      ...
                      int multiThreadLevel, JobContext*& jobContext)
{
    ...
    jobContext->atomic_counter = new std::atomic<uint32_t>(0);
...
void set_counter(std::atomic<uint32_t>* atomic_counter,stage_t stage)
{
    // Zero the first 30 bits and preserve the last 2 bits
    uint32_t new_value =  static_cast<uint32_t>(stage)<<30 & static_cast<uint32_t>(0x03)<<30;
